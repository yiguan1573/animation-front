...
export default {
  ...
  mounted () {
    ...
    // 缓存容器宽度
    this.barMainWidth = this.barrageMainDm.clientWidth==0?1920:this.barrageMainDm.clientWidth;
    // 初始化弹幕dom组
    this.init();
    // 开始播放弹幕
    this.playDm();
    ...
    playDm () {
      ...
      self.intervalDM = setInterval(() => {
        ...
        if (self.barrages.length > 0) {
          ...
          if (typeof(barrage) !== 'undefined' && barrage.direction == 'top' && (topChannel = self.getTopChannel()) != -1) {
            // top....
            self.shootTopDanmu(barrage, topChannel);
          }
					
          if (typeof(barrage) !== 'undefined' && barrage.direction == 'default' && (channel = self.getChannel()) != -1) {
            // default....
            let domItem = self.getFreeChannelDom(channel);
            if (domItem) {
              // let danmu = self.barrages.shift();
